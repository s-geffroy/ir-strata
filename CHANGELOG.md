...
- **Re-sourçage des scores (démarrage)** : champ `calibration_status` (`seed`/`sourced`) ; rapport d'avancement `scripts/calibration_report.py` → `docs/CALIBRATION_STATUS.md`. Première tranche re-sourcée comme gabarit : **1945-1962, couche `strategic_reality` (7 scores)**, justifications réelles adossées aux références vérifiées (Waltz, Gilpin, Jervis, Ruggie, Bull, Wallerstein, Said…). Avancement : 7/1549 scores actifs (0,5 %).
